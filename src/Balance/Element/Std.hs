...
data StdElParams e a = FillElParams (FillParams a)
                     ...
                     | LayersElParams (LayersParams e a)
                     | StackElParams (StackParams e a)
                     | VBoxElParams (VBoxParams e a)

deriving instance ( Eq a,   Eq   (Params e a) ) => Eq   (StdElParams e a)
deriving instance ( Show a, Show (Params e a) ) => Show (StdElParams e a)
deriving instance ( Read a, Read (Params e a) ) => Read (StdElParams e a)
deriving instance Functor     (Params e) => Functor     (StdElParams e)
deriving instance Foldable    (Params e) => Foldable    (StdElParams e)
deriving instance Traversable (Params e) => Traversable (StdElParams e)
...
instance RectangularElement e => Element (StdElF a e) where
  ...
  penalty (LayersEl e) (LayersElParams ps) = penalty e ps
  penalty (StackEl e)  (StackElParams ps)  = penalty e ps
  penalty (VBoxEl e)   (VBoxElParams ps)   = penalty e ps
  penalty _            _                   = error "StdEl param type does not match element type or there is no penalty case for a given element type"
  ...
  guess pxy (LayersEl e) = LayersElParams (guess pxy e)
  ...
  render (LayersEl e) (LayersElParams p) s = render e p s
...
layersElPxy :: LayersParams e a -> Proxy (Layers e a)
layersElPxy _ = Proxy
...
getBoundingBox (LayersElParams p) = p ^. boundingBox (layersElPxy p)
...
setBoundingBox (LayersElParams p) bb = LayersElParams $ set (boundingBox (layersElPxy p)) bb p
